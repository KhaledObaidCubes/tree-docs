...
function buildParse(options = {}) {
    ...
    return {
        parse(src) {
            return parser.parse(src, options);
        }
    };
}
...
function normalizeSfcComponent(code, config = {}) {
    const { script, scriptSetup, template, styles } = vueInbrowserCompilerUtils.parseComponent(code);
...
function compileVueCodeForEvalFunction(code, config = {}, scopeId) {
    ...
    const compiledComponent = Object.assign(Object.assign({}, nonCompiledComponent), { scopeId, script: sucrase.transform(nonCompiledComponent.script, configWithTransforms).code });
...
function prepareVueCodeForEvalFunction(code, config) {
    ...
    let offset = 0;
    const varNames = [];
    walkes__default["default"](ast.program, Object.assign({ 
...
function getImports(code) {
    ...
    try {
        const ast = getAst(code);
        walkes__default["default"](ast.program, {
            ImportDeclaration(node) {
